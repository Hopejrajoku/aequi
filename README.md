...
Plaintext
aequi/
├── sdk/              # The Core TypeScript Engine
│   ├── src/
│   │   ├── config.ts # Network & Contract Constants
│   │   ├── index.ts  # Transaction Builders
│   │   └── test.ts   # Integration Test Suite
│   └── package.json  # SDK Dependencies (Sui v2.6.0)
├── demo-bot/         # Implementation Example
└── Move/             # Smart Contract Source (Sui Move)
## Core Features
...
### SDK Usage
Aequi uses a Dynamic Discovery Pattern to ensure compatibility with modern JavaScript runtimes.
...
Ensure npm test passes in the sdk/ directory.
